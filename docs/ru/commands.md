...
!!! info "Вещи которые стоит учесть:"
    ...
    * На данный момент настройки чат-команд возвращаются на стандартные значения по окончании матча, однако команды-переключатели (true/false) могут сохранять значение до тех пор пока сервер не перезапустится; нужно иметь это в виду при изменении тех или иных настроек.
...
| [](){ #zed-time-effect }zedtimeeffect  | zte | `<true/false>` | Если стоит на `false`, то отключает эффект выцвевшей картинки во время зед-тайма; по умолчанию эффект включен (`true`) |

!!! info "Настройки команд, касающихся лежащего на земле оружия, выпадения предметов и эффекта зед-тайма, сохраняются локально в клиентских конфигах, поэтому для их корректной работы убедитесь что ваши конфиги не находятся в режиме 'только чтения'."
...
??? question "Как работает система блокировки оружия?"
    ...
    т.к. множество ничего не подозревающих людей часто не понимают что им не следует подбирать или продавать чужое оружие.
...
| [](){ #kill-zeds }killzeds | kz | | Убивает 5 живых зедов; имеет 120-секундный кулдаун на использование. Применяется в случаях когда зеды застревают где-то на карте |
...
[^1]: Биги (Larges) — Скрейки и Фишпаунды, медиумы (Mediums) — Хаски, Сирены, Толстяки и Берсеркеры.