...
При зажатии `J` откроется круговое меню с выбором чат-команд. При желании клавишу можно переназначить на другую, отредактировав файл `KFInput.ini`. Это всё ещё можно сделать консольной командой `setbind <кнопка> GBA_ShowVoteComms`, однако я рекомендую делать это вручную через файл `KFInput.ini`, потому что есть шанс, что в противном случае строчки с клавишами продублируются и будут считываться некорректно при повторном заходе на серверы.

??? question "Как изменить ^^KFinput.ini^^?"

    * Сначала убедитесь что игра не запущена;

    * Откройте папку `..\Documents\My Games\KillingFloor2\KFGame\Config`;

	* В ней будет файл `KFinput.ini`;

	* Используя поиск (Ctrl+F), найдите следующую строчку:
    ...
    * Измените клавишу с `J` на любую другую в зависимости от ваших предпочтений.

	!!! warning "В этом файле есть ещё одна похожая строка, в которой есть `GBA_ShowVoteComms` в поле `Name=`. Не изменяйте её, т.к. этим можно сломать функционал работы менюшки."
...
В ванильном матче при 6 игроках это значение ограничено до 32 (36 на SPB-GS), в то время как стандартное значение для HoE+ - 56.

* Общая идея ручной настройки MaxMonsters заключается в том чтобы значение было выше чем ванильное, но достаточное для хорошего потока зедов по ходу волны. 
...
* По нашему опыту holdout-карты по типу Энергоядра (Power Core) играются лучше на 48mm (и менее) потому что зачастую при высоком значении `mm` спавны на них ощущаются чересчур интенсивными, а также наблюдаются проблемы с производительностью, в то время как на просторных картах можно комфортно играть при значениях 64mm и выше.
