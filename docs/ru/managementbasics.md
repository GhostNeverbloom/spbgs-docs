...
??? question "Как изменить ^^KFinput.ini^^?"
    ...
    * Измените клавишу с `J` на любую другую в зависимости от ваших предпочтений.
	...
	!!! warning "В этом файле есть ещё одна похожая строка, в которой есть `GBA_ShowVoteComms` в поле `Name=`. Не изменяйте её, т.к. это можно сломать функционал работы менюшки."
